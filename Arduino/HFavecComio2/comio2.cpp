 ...
 * \param     op    operation à réaliser (sur 4 bits : valeur de 0 à 16) / valeurs définie : OP_READ, OP_WRITE, OP_FUNCTION
 ...
 * \param     val   valeur numérique sur 16 bits à retourner (donnée de l'adresse, valeur de la PIN, ...)
 */
{
 ...
 * \param     nerr   numéro d'erreur à retourner sur 8 bits.
 */
{
  writeF('!');
  writeF(nerr);
...
/**
 * \brief     lecture d'une trame "commande" de type "Comio1" dans le buffer d'entrée.
...
/**
 * \brief     réalise l'opération demandé par une commande de type "Comio1".
 ...
 */
{
  ...
  _comio_debut_trame(id, COMIO2_CMD_ERROR, 1, &cchecksum);
  cchecksum+=error;
  writeF(error);
  _comio_fin_trame(&cchecksum);
}
 ...
 * \param     l_data nombre de caractère de la zone data (max 80 octets)
 * \return    0 => trame correcte,  < 0 => erreur
 */
{
  /*
   Structure d'une trame de type "Comio2"
  ...
  while(stat<6)
  {
    ...
    for(int i=0;i<5;i++)
    {
      ...
      if(c>=0)
        break;
      delay(1); // 5 milliseconde entre 2 lectures
    }

    if(c<0)
      return -1; // pas de données => sortie direct "sans rien dire"

    // automate à états pour traiter les différents champs de la trame
    switch(stat)
    {
    case 0: // lecture de la longueur de trame
      *l_data=(unsigned char)c-6;
    ...
    case 5:
      if(c!=']')
        return -2; // pas de fin de trame correct ... on oubli tout et on sort
...
void Comio2::_comio_debut_trame(unsigned char id, unsigned char cmd, int l_data, int *cchecksum)
{
    writeF('{');
...
void Comio2::_comio_fin_trame(int *cchecksum)
{
 ...
 */
{
  ...
  case COMIO2_CMD_READMEMORY: // memory read - data contient la liste des cases mémoires à lire
    ...
    _comio_debut_trame(id, COMIO2_CMD_READMEMORY, l_data, &cchecksum);
  ...
  case COMIO2_CMD_WRITEMEMORY: // memory write - data contient la liste des couples Adresse/valeur
    ...
    _comio_debut_trame(id, COMIO2_CMD_WRITEMEMORY, 1, &cchecksum);
    cchecksum+=j;
  ...
  case COMIO2_CMD_CALLFUNCTION: // memory write - data contient la liste des couples Adresse/valeur
    if(l_data < 1 || data[0]>=COMIO2_MAX_FX)
    {
      _comio_send_errorV2(id, COMIO2_ERR_PARAMS);
      return -1;
    }
    if(comio_functionsV2[data[0]])
    {
      ...
      if(id)
      {
        _comio_debut_trame(id, COMIO2_CMD_CALLFUNCTION, 1, &cchecksum);
        cchecksum+=retval;
        writeF(retval); // nombre d'octets écrit
        _comio_fin_trame(&cchecksum);
      }
    }
    else
